            ...
            <div class="card-header card-header-tabs card-header-primary">
              <div class="nav-tabs-navigation">
                <div class="nav-tabs-wrapper">
                  <span class="nav-tabs-title">Detalle Permisos Vicepresidencia Finanzas</span>
    </div></div></div></div></div>
            ...
            <div class="card-body table-responsive">
              <table class="table table-hover">
                ...
                @php
                    ...
                    $totalvpfc = 0;
                  @endphp
                @foreach($departamentoVPF as $dep)       
                ...
                <tr>
                    ...
                    @forelse ($permisosVPFA as $vpfa)
                      @if($dep->Puesto == $vpfa->Puesto)
                        <td align='center'>{!! $vpfa->vpfa !!}</td>
                        @php
                          $totalvpfa=$totalvpfa + $vpfa->vpfa;
                          $valor1=1;
                        @endphp
                      @else
                      @endif  
                    ...
                    @forelse ($permisosVPFD as $vpfd)
                      @if($dep->Puesto == $vpfd->Puesto)
                        <td align='center'>{!! $vpfd->vpfd !!}</td>
                        @php
                          $totalvpfd=$totalvpfd + $vpfd->vpfd;
                          $valor2=1;
                        @endphp
                      @else
                      @endif  
                    ...
                    @forelse ($permisosVPFC as $vpfc)
                      @if($dep->Puesto == $vpfc->Puesto)
                        <td align='center'>{!! $vpfc->vpfc !!}</td>
                        @php
                          $totalvpfc=$totalvpfc + $vpfc->vpfc;
                          $valor3=1;
                        @endphp
                      @else
                      @endif  
                    ...
                    @forelse ($permisosVPFP as $vpfp)
                      @if($dep->Puesto == $vpfp->Puesto)
                        <td align='center'>{!! $vpfp->vpfp !!}</td>
                        @php
                          $totalvpfp=$totalvpfp + $vpfp->vpfp;
                          $valor4=1;
                        @endphp
                      @else
                      @endif  
                ...
                @endforeach 
                  <tr>
                    ...
                    <td align='center'><b>{{ number_format($permisosVPF,0) }}</b></td>
                    
                    <td align='center'><b>{{ number_format($totalvpfa,0) }}</b></td>
                    <td align='center'><b>{{ number_format($totalvpfd,0) }}</b></td>
                    <td align='center'><b>{{ number_format($totalvpfc,0) }}</b></td>
                    <td align='center'><b>{{ number_format($totalvpfp,0) }}</b></td>
                  </tr>
        ...
        <div class="col-lg-3 col-md-4 col-sm-4">
          <div class="card card-stats">
            <div class="card-header card-header-warning card-header-icon">
              ...
              <input type="hidden" id="vacaciones" name="vacaciones" value={{ $permisosVPF }} />
        ...
        <div class="col-lg-3 col-md-4 col-sm-4">
          <div class="card card-stats">
            <div class="card-header card-header-success card-header-icon">
              ...
              <input type="hidden" id="permisos" name="permisos" value={{ $permisosVPF }} />
            </div>
...
<script>
  ...
  $('#fecha_final').change(function () {
    window.location.href="fechasVPF2?inicial="+$('#fecha_inicial').val()+"&final="+$('#fecha_final').val();
