            ...
            <div class="card-header card-header-tabs card-header-primary">
              <div class="nav-tabs-navigation">
                <div class="nav-tabs-wrapper">
                  <span class="nav-tabs-title">Detalle Permisos Vicepresidencia Ventas</span>
    </div></div></div></div></div>
            ...
            <div class="card-body table-responsive">
              <table class="table table-hover">
                ...
                <tr>
                    ...
                    @forelse ($permisosVPVA as $VPVa)
                      @if($dep->Puesto == $VPVa->Puesto)
                        <td align='center'>{!! $VPVa->vpva !!}</td>
                        @php
                          $totalVPVa=$totalVPVa + $VPVa->vpva;
                          $valor1=1;
                        @endphp
                      @else
                      @endif  
                    ...
                    @forelse ($permisosVPVD as $VPVd)
                      @if($dep->Puesto == $VPVd->Puesto)
                        <td align='center'>{!! $VPVd->vpvd !!}</td>
                        @php
                          $totalVPVd=$totalVPVd + $VPVd->vpvd;
                          $valor2=1;
                        @endphp
                      @else
                      @endif  
                    ...
                    @forelse ($permisosVPVC as $VPVc)
                      @if($dep->Puesto == $VPVc->Puesto)
                        <td align='center'>{!! $VPVc->vpvc !!}</td>
                        @php
                          $totalVPVc=$totalVPVc + $VPVc->vpvc;
                          $valor3=1;
                        @endphp
                      @else
                      @endif  
                    ...
                    @forelse ($permisosVPVP as $VPVp)
                      @if($dep->Puesto == $VPVp->Puesto)
                        <td align='center'>{!! $VPVp->vpvp !!}</td>
                        @php
                          $totalVPVp=$totalVPVp + $VPVp->vpvp;
                          $valor4=1;
                        @endphp
                      @else
                      @endif  
                ...
                @endforeach 
                  <tr>
                    ...
                    <td align='center'><b>{{ number_format($permisosVPV,0) }}</b></td>
                    
                    <td align='center'><b>{{ number_format($totalVPVa,0) }}</b></td>
                    <td align='center'><b>{{ number_format($totalVPVd,0) }}</b></td>
                    <td align='center'><b>{{ number_format($totalVPVc,0) }}</b></td>
                    <td align='center'><b>{{ number_format($totalVPVp,0) }}</b></td>
                  </tr>
        ...
        <div class="col-lg-3 col-md-4 col-sm-4">
          <div class="card card-stats">
            <div class="card-header card-header-warning card-header-icon">
              ...
              <input type="hidden" id="vacaciones" name="vacaciones" value={{ $permisosVPV }} />
...
<script>
  ...
  $('#fecha_final').change(function () {
    window.location.href="fechasVPV2?inicial="+$('#fecha_inicial').val()+"&final="+$('#fecha_final').val();
